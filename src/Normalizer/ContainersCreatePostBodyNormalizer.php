    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('volumes') && null !== $data->getVolumes()) {
            ...
            foreach ($data->getVolumes() as $key_1 => $value_3) {
                $values_3[$key_1] = null === $value_3 ? null : new \Docker\API\Runtime\JsonObject($this->normalizer->normalize($value_3, 'json', $context));
            }
        ...
        if ($data->isInitialized('networkingConfig') && null !== $data->getNetworkingConfig()) {
            $dataArray['NetworkingConfig'] = null === $data->getNetworkingConfig() ? null : new \Docker\API\Runtime\JsonObject($this->normalizer->normalize($data->getNetworkingConfig(), 'json', $context));
        }
        foreach ($data->additionalPropertyEntries() as $key_3 => $value_8) {
            if (preg_match('/.*/', (string) $key_3)) {
